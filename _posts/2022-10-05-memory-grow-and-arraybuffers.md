...
As far as Rust (WebAssembly) is concerned, everything is fine; however, the JavaScript host environment sees that shared memory has changed size, so throws away its old `ArrayBuffer` and helpfully creates you a new one.
...
1. We assume that, after calling a Wasm function, something bad will probably have happened to our shared memory access objects, and just code around it;[^2] or
...
[^2]: Changes to the size of WebAssembly memory can be detected in JavaScript by monitoring at the value of `WebAssembly.Memory.length`.
   If this value grows, then you will need to redefine any object that overlaid shared memory
